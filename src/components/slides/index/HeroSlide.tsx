...
export function HeroSlide({ index }: HeroSlideProps) {
  ...
  return (
    ...
    >
      <div
        className="md:h-auto md:aspect-[3/4] glass-panel overflow-hidden flex flex-col group md:max-w-xl md:p-12 w-full h-full max-w-none rounded-none px-5 pb-5 pt-12 relative shadow-2xl justify-between card-bg"
      ...
      >
        ...
        <div className="flex-1 flex flex-col z-10 relative justify-center items-center text-center">
          ...
          {/* Simplified headline - ONE clear value proposition */}
          <AnimatedElement delay={0.3}>
            <h1 className="text-3xl md:text-4xl leading-[0.95] font-bold text-white tracking-tight font-display mb-6 md:mb-8">
              Be the Agent Your
              <span className="block text-teal-300 drop-shadow-md">Clients Rave About</span>
            </h1>
          </AnimatedElement>
          ...
          <AnimatedElement delay={0.4} className="mb-6 md:mb-8">
            <div className="inline-flex items-center gap-2 bg-black/60 backdrop-blur-sm border border-white/10 rounded-full px-4 py-2">
              <span className="text-teal-300 font-bold font-display drop-shadow-md">{heroStat.value}</span>

...
export function CaseStudiesSlide({ index }: CaseStudiesSlideProps) {
  return (
    ...
    >
      <div
        className="md:h-auto md:aspect-[3/4] glass-clear overflow-hidden flex flex-col md:max-w-xl md:pt-12 md:pr-12 md:pl-12 w-full h-full max-w-none rounded-none pt-20 pr-6 pb-12 pl-6 relative shadow-2xl justify-start card-bg"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <AnimatedElement delay={0.2} className="shrink-0">
            <h2 className="text-4xl md:text-5xl font-normal tracking-tight text-white mb-6 md:mb-8 drop-shadow-md font-display">
              Real Results from
              <br />
          ...
          <div className="flex flex-col gap-4 w-full flex-1 min-h-0 overflow-y-auto scrollbar-hide pb-4">
            {caseStudies.map((study, i) => (
              <AnimatedElement
                key={i}
                delay={0.3 + i * 0.1}
                className="bg-black/40 backdrop-blur-md border border-white/10 p-5 w-full hover:bg-black/50 transition-colors duration-500 shrink-0"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center border border-white/10">
                      <span className="text-white font-medium">{study.initials}</span>
                    </div>
                    <div>
